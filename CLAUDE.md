...
- **Duplicate check runs on the AI-normalized (and article-corrected) headword, not the user's raw input.** The model may correct spelling/casing, normalize an inflected form, or add the Dutch article — checking the raw input would miss a duplicate already in the set under its canonical form. A match throws `conflict()` (409) before any DB write. **Under `keepHeadword` the check runs on the caller's headword instead**, since that is what gets stored — the scope is still set-scoped either way.
- **A 502 `GEMINI_INCOMPLETE`** is thrown if the model's response is missing `wordTranslated` or `definition` (only the ones the caller did not supply — see the caller-supplied bullet below) — those are the two fields the client depends on to render anything, so a partial response fails loudly instead of inserting a half-populated word.
...
- **A caller that already has the translation and gloss may supply them** —
  `wordTranslated`, `definition`, `posHint`, `pinned` and `keepHeadword` are optional body
  fields on
...
- **`keepHeadword` pins the caller's headword, in code and not only in the prompt.**
  Normally the model re-lemmatises: a plural becomes its singular, an inflected adjective
  its base form. That is right for typed input, where an inflected form is a typo, and
  wrong for a curated list — the Dutch exam page teaches `de boodschappen` and
  `naar bed gaan` as themselves, 43 of its 446 addable words being plurals or expressions.
  With the flag, `buildPrompt` states the headword as already known, drops it from the
  requested fields, and suppresses BOTH the `HEADWORD NORMALIZATION` block and the
  headword-shaping clauses in `DUTCH-SPECIFIC GRAMMAR RULES` (leaving those in produced
  grammar and examples for `gaan` rather than `naar bed gaan`); `generateWordForSet` then
  uses the caller's form regardless of what the model returns. Without the flag every
  prompt is byte-identical to before it existed — there is a snapshot test pinning that.
...
- **A selected box `6` means `leitnerBox >= 6`.** Mastered words sit in boxes 7+ (`leitner.intervalDaysForBox`'s mastered stages) and the app's chip row / `AppTheme.Colors.lifecycle` both stop at 6, so folding them into the top chip is what keeps them reachable at all. Absent/empty `boxes` means every box.
- **Five recency filters, no silent fallback:** `all`, `lastTime`, `week`, `month`, `ever`. `lastTime` reads the single most recent attempt (so a never-answered word is excluded — it wasn't answered wrongly, it wasn't answered); `week`/`month` are `EXISTS` over wrong attempts since a **start-of-day-aligned** cutoff (`startOfDayAfterDays(-7|-30)` in `APP_TIMEZONE`), not an hour-precise one, so "this past week" can't mean something different depending on what time the app was opened. An unknown value is a `400`.
- **`learning.resetProgress` also calls `recall.clearForScope`** — the one place Recall and Leitner state are cleared together, because "reset progress" is a scope-level "start over". Without it the recency filters would keep reporting mistakes made before the reset. `resetProgress` now returns `{ reset, recallCleared }` (additive; the app ignores the body).
- **Recall history is private to Recall.** It never reads `lapseCount`/`openLapse`/`lastLapsedAt`, and Trouble Words never reads `recall_attempts`.
