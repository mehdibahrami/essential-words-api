...
- **Dutch verb conjugation is never asked of the model.** Present tense is already rule-derived live on every read (`words.buildGrammar` → `dutchGrammar.buildVerbGrammar`, see the Grammar section above), so trusting an LLM-generated conjugation table would be strictly worse than the existing, verified engine. The prompt only asks Gemini for a `grammar` object — `{article, plural}` — when the language is Dutch (`nl-NL`) **and** the model itself classified the word as a noun, since gender/plural are lexical facts no rule can derive. That's the one grammar fact `extractGrammar()` will store; everything else (including any verb grammar the model might volunteer) is discarded.
- **Duplicate check runs on the AI-normalized headword, not the user's raw input.** The model may correct spelling/casing or add the Dutch article (e.g. user types "huis", headword comes back "het huis" per the prompt's explicit instruction) — checking the raw input would miss a duplicate already in the set under its canonical form. A match throws `conflict()` (409) before any DB write.
